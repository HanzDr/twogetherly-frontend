...
const initialPosts: DiaryPost[] = [
  {
    ...
    initials: "F",
    message:
      "I loved our quiet coffee date this morning. Here's to more slow Sundays together. ❤️",
  ...
  },
  {
    ...
    initials: "Y",
    message:
      "A little reminder: I'm proud of everything we're building, one day at a time.",
  ...
  return (
    <main className="min-h-screen bg-linear-to-b from-rose-50 via-white to-pink-50 px-5 py-8 sm:px-8">
      <div className="mx-auto max-w-3xl">
        <Link
          to="/dashboard"
          className="mb-7 inline-flex items-center gap-2 text-sm font-medium text-rose-600 hover:text-rose-700"
        >
        ...
        <header className="mb-8 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-rose-950/55">
              A private little space for you and {partnerName}.
            </p>
          </div>
        </header>
          ...
          <CardContent className="p-5">
            <form onSubmit={publishPost}>
              <div className="flex gap-3">
                <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-rose-500 font-semibold text-white">
                  Y
                </span>
                <textarea
                  value={message}
              ...
              {photo && (
                <div className="relative mt-4 overflow-hidden rounded-2xl border border-rose-100 sm:ml-13">
                  <img
                    src={photo}
                    alt="Selected diary upload preview"
              ...
              {error && (
                <p className="mt-3 text-sm text-red-600 sm:ml-13" role="alert">
                  {error}
                </p>
              )}
              ...
              <div className="mt-4 grid gap-3 sm:ml-13 sm:grid-cols-2">
                <div className="relative">
                  <MapPin
                    className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-rose-400"
                    aria-hidden="true"
                  />
                  <input
                    value={location}
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-rose-100 pt-4 sm:ml-13">
                <input
                  ...
                  className="sr-only"
                  aria-label="Choose a diary photo"
                />
                <Button
                  type="button"
                  variant="ghost"
          ...
          {posts.map((post) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className="flex size-10 items-center justify-center rounded-full bg-linear-to-br from-rose-400 to-pink-600 font-semibold text-white">
                  {post.initials}
                </span>
                <div>
                  <p className="font-semibold text-rose-950">{post.author}</p>
                  <time className="text-xs text-rose-950/40">
                    {post.createdAt}
                  </time>
                </div>
              </div>
              {post.message && (
                <p className="mt-4 whitespace-pre-wrap leading-7 text-rose-950/75">
                  {post.message}
                </p>
              )}
              {post.photo && (
                <img
                  src={post.photo}
                  alt={`Memory shared by ${post.author}`}
                  className="mt-4 max-h-[32rem] w-full rounded-2xl object-cover"
                />
              )}
